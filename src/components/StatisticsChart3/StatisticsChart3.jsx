...
const StatisticsChart3 = ({ themeMode }) => {
  ...
  const fetchData = () => {
    setIsLoading(true);
    const startDate = formatDate(dateRange[0]); // Format the start date
    ...
    axios
      ...
      .then((response) => {
        ...
        if (Array.isArray(data)) {
          const officeDataMap = new Map(); // Map to store office data by office name
          let totalCapacity = 0; // Variable to store the total capacity
          ...
          data.forEach((item) => {
            ...
            if (officeDataMap.has(officeName)) {
              // If office name already exists in the map, update its data
              const officeData = officeDataMap.get(officeName);
              officeData.currentStock += currentStock;
              officeData.capacity += capacity;
            } else {
              // If office name does not exist in the map, create a new entry
              officeDataMap.set(officeName, { officeName, currentStock, capacity });
            }
  
            totalCapacity += capacity; // Update the total capacity
          });
  
          const filteredData = Array.from(officeDataMap.values()).map((officeData) => {
            const { officeName, currentStock, capacity } = officeData;
            const requirementPercentage = ((totalCapacity - currentStock) / totalCapacity) * 100;
            console.log(`Requirement Percentage for ${officeName}: ${requirementPercentage}%`);
  ...
  const option = {
    ...
    tooltip: {
      trigger: "axis",
      formatter: "<b>Office:</b> {b} <br> <b>Requirement Percentage:</b> {c} ",
    },
    yAxis: {
      type: "category",
      name: "Office Name",
